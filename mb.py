...
class TurtleShapes(Node):
    ...
    def move_circle(self, speed=1.0, radius=0.5, duration=3.14):
        speed=1.0
        duration=3.14
        radius=0.5
        twist = Twist()
    ...
    def move_triangle(self, speed=1.0, side_length=2.0):
        ...
        # Turn -150 degrees
        twist.linear.x = 0.0
        twist.angular.z = math.radians(-150)
        start_time = time.time()
        while time.time() - start_time < 1.0:
            self.pub.publish(twist)
            time.sleep(0.1)
        ...
        twist.angular.z = 0.0
        start_time = time.time()
        while time.time() - start_time < side_length / (speed*2):
            self.pub.publish(twist)
            time.sleep(0.1)
        self.stop()
        time.sleep(0.5)
        ...
        twist.angular.z = 0.0
        start_time = time.time()
        while time.time() - start_time < side_length / (speed*1):
            self.pub.publish(twist)
            time.sleep(0.1)
        self.stop()
        time.sleep(0.5)
